...
void CTerrain::Render_Terrain()
{
	...
	D3DXMatrixTranslation(&matTrans, TutorialSize.x / 2 - m_pView->GetScrollPos(0), TutorialSize.y / 2 - m_pView->GetScrollPos(1), 0.f);
	...
	for (auto& pTile : m_vecTile)
	{
		...
		_matrix matScale, matTrans, matWorld;
		D3DXMatrixScaling(&matScale, pTile->vSize.x * 2, pTile->vSize.y * 2, 0.f);
		D3DXMatrixTranslation(&matTrans, pTile->vPos.x - m_pView->GetScrollPos(0), pTile->vPos.y - m_pView->GetScrollPos(1), 0.f);
		matWorld = matScale * matTrans;
