...
HRESULT CDarkCow::Ready_GameObject()
{
...
int CDarkCow::Update_GameObject()
{
...
void CDarkCow::LateUpdate_GameObject()
{
	if (!m_isSearch)
	{
...
void CDarkCow::Render_GameObject()
{

	pTexInfo = CTexture_Manager::Get_Instance()->Get_TexInfo(L"DarkCow", m_szFrameKey, DWORD(m_tFrame.fFrameStart));
...
void CDarkCow::Release_GameObject()
{
}


void CDarkCow::MoveFrame(_float fSpeed)
{
...
CGameObject* CDarkCow::Create(LPVOID* pArg)
{
	CGameObject* pInstnace = new CDarkCow;
	if (FAILED(pInstnace->Ready_GameObject()))
		return nullptr;
	return pInstnace;
}

CGameObject* CDarkCow::Create(_vec3 vpos)
{
	CGameObject* pInstnace = new CDarkCow;
	if (FAILED(pInstnace->Ready_GameObject()))
		return nullptr;
